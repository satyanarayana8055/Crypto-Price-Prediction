...
from src.pipeline.feature_extraction import extract_features
from src.pipeline.model import train_model
...
def create_coin_tasks(coin: str):
    ...
    train_task = PythonOperator(
        ...
        op_kwargs={
            "coin": coin,
            "hyperparams": config["airflow"]["model_dag"]["hyperparams"],
        },
        dag=dag,
    )
...
coin_tasks = [
    create_coin_tasks(coin) for coin in config["airflow"]["model_dag"]["coins"]
]
