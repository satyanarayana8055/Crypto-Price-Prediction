...
class NotifyService:
    ...
    def get_default_config(self):
        ...
        return {
            ...
            "thresholds": {
                "accuracy_drop": float(THRESHOLDS.get("r2") or 0.05),
                "mse_increase": float(THRESHOLDS.get("mse") or 0.20),
            },
            "coins": COINS,
            "check_interval": 3600,
        }
    ...
    def check_thresholds(self, coin, current_metrics: dict, previous_metrics: dict):
        ...
        if "accuracy" in current_metrics:
            if previous_metrics and "accuracy" in previous_metrics:
                accuracy_drop = (
                    previous_metrics["accuracy"] - current_metrics["accuracy"]
                )
        ...
        if "mse" in current_metrics and previous_metrics and "mse" in previous_metrics:
            mse_increase = (
                current_metrics["mse"] - previous_metrics["mse"]
            ) / previous_metrics["mse"]
