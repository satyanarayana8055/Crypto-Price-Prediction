...
def test_model_prediction(coin):
    ...
    perf_path = os.path.join(DATA_PATHS['performance_metrics'],f'{coin}_performance_metrics.csv')
    ...
    assert isinstance(predictions[0], (float, int, np.float64, np.float32)), "Prediction is not numeric" 
